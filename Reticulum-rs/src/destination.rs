...
impl DestinationAnnounce {
    pub fn validate(packet: &Packet) -> Result<(SingleOutputDestination, &[u8]), RnsError> {
        if packet.header.packet_type != PacketType::Announce {
            return Err(RnsError::PacketError("Invalid packet type".to_string()));
        }
        ...
        let verifying_key = {
            ...
            VerifyingKey::from_bytes(&key_data).map_err(|e| RnsError::CryptoError(format!("Failed to parse verifying key: {:?}", e)))?
        };
        ...
        let signature = Signature::from_slice(signature).map_err(|e| RnsError::CryptoError(format!("Failed to parse signature: {:?}", e)))?;
