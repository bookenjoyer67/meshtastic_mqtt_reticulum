...
impl<R: CryptoRngCore + Copy> Fernet<R> {
    ...
    pub fn encrypt<'a>(
        ...
        if out_buf.len() <= FERNET_OVERHEAD_SIZE {
            return Err(RnsError::invalid_argument("Output buffer too small for Fernet token"));
        }
        ...
        let mut hmac = <HmacSha256 as Mac>::new_from_slice(&self.sign_key)
            .map_err(|_| RnsError::invalid_argument("Invalid HMAC key length"))?;
    ...
    pub fn verify<'a>(&self, token: Token<'a>) -> Result<VerifiedToken<'a>, RnsError> {
        ...
        if token_data.len() <= FERNET_OVERHEAD_SIZE {
            return Err(RnsError::invalid_argument("Token data too small for Fernet verification"));
        }

        let expected_tag = &token_data[token_data.len() - HMAC_OUT_SIZE..];

        let mut hmac = <HmacSha256 as Mac>::new_from_slice(&self.sign_key)
            .map_err(|_| RnsError::invalid_argument("Invalid HMAC key length for verification"))?;
    ...
    pub fn decrypt<'a, 'b>(
        ...
        if token_data.len() <= FERNET_OVERHEAD_SIZE {
            return Err(RnsError::invalid_argument("Token data too small for Fernet decryption"));
        }
        ...
        let msg = AesCbcDec::new(&self.enc_key, &iv.into())
            ...
            .map_err(|_| RnsError::crypto_error("Failed to decrypt Fernet token"))?;
