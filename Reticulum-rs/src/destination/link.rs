...
impl Link {
    ...
    pub fn new_from_request(
        ...
        if packet.data.len() < PUBLIC_KEY_LENGTH * 2 {
            return Err(RnsError::invalid_argument("Packet data too small for link request"));
        }
    ...
    pub fn data_packet(&self, data: &[u8]) -> Result<Packet, RnsError> {
        if self.status != LinkStatus::Active && self.status != LinkStatus::Stale {
            ...
            return Err(RnsError::link_closed("Cannot create data packet for closed link"));
        }
...
fn validate_proof_packet(
    ...
    if packet.data.len() < MIN_PROOF_LEN {
        return Err(RnsError::packet_error("Proof packet too short"));
    }
    ...
    let signature = Signature::from_slice(&packet.data.as_slice()[..SIGNATURE_LENGTH])
        .map_err(|_| RnsError::crypto_error("Failed to parse signature"))?;
...
fn validate_message_proof(
    ...
    if data.len() <= HASH_SIZE {
        return Err(RnsError::PacketError("Invalid packet type for link".to_string()));
    }

    let maybe_signature = Signature::from_slice(&data[HASH_SIZE..]);
    let signature = match maybe_signature {
        Ok(s) => s,
        Err(_) => return Err(RnsError::packet_error("Failed to parse signature from packet data")),
    };
